...
def get_users_with_any_enrollment_count() -> int:
    with Session(models.engine) as session:
        users_with_any_enrollment_count = session.query(
            func.count(distinct(models.Enrollment.tg_id))
        ).scalar() or 0
    logger.info(f"{users_with_any_enrollment_count=}")
    return users_with_any_enrollment_count


def get_users_with_active_enrollment_count() -> int:
    with Session(models.engine) as session:
        users_with_active_enrollment_count = (session.query(func.count(distinct(models.Enrollment.tg_id)))
                .join(models.Course, models.Course.id == models.Enrollment.course_id)
                .filter(models.Course.is_active.is_(True)).scalar() or 0)
    logger.info(f"{users_with_active_enrollment_count=}")
    return users_with_active_enrollment_count
...
async def calculate_metrics_and_report(bot: Bot = None) -> None:
    ...
    try:
        ...
        # users enrolled to any course
        metrics.set("users_with_any_enrollment_total", get_users_with_any_enrollment_count())
        metrics.set("users_with_active_enrollment_total", get_users_with_active_enrollment_count())
